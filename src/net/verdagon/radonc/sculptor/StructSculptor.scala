...
object StructSculptor {
  ...
  private def signatureToString(paramTypes: List[Reference3], returnType3: Reference3): String = {
  ...
  private def declareStructInfo(program: Program3, struct: StructDefinition3): String = {
    ...
    val structName = struct.globalName
    val members =
      struct.members.map(m => "  , " + TypeSculptor.getMemberTypeLlvmType(m.tyype) + " ; " + m.name).mkString("\n")

    val (methodsType, methodsValue) = assembleMethodsTypeAndValue(program, struct, struct)
    ...
    val superclassMethodsValue =
      struct.base match {
        ...
        case Some(superstructRef) => {
          ...
          val (superclassMethodsValue, _) = assembleMethodsTypeAndValue(program, superstructDef, struct)
  ...
  private def assembleMethodsTypeAndValue(program: Program3, baseOrThisStruct: StructDefinition3, thisStruct: StructDefinition3): (String, String) = {
